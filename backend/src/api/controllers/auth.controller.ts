...
export const signup=async(req:Request,res:Response)=>{
  try{
      const{name,userName,Gender,password,role,email,phone,state}=req.body
    ...
    const newUser=await User.create({name,userName,Gender,password:hashedPassword,role:role || "User",email,phone,state})
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    // Generate JWT
    const token = jwt.sign({ id: user.id, role: user.role, state: user.state}, process.env.JWT_SECRET || "secret", {
      expiresIn: "1d",
    });
